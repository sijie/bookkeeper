...
public class DeleteTableCommand extends AdminCommand<Flags> {
    ...
    protected void run(StorageAdminClient admin,
        ...
        spec.console().println("Deleting table '" + tableName + "' ...");
        try {
            result(admin.deleteStream(globalFlags.namespace, tableName));
            spec.console().println("Successfully deleted table '" + tableName + "'.");
