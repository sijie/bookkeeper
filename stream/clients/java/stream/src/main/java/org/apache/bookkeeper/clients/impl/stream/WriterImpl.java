...
import org.apache.bookkeeper.clients.impl.stream.range.StreamRangeEventWriterImpl;
...
class WriterImpl<KeyT, ValueT>
    ...
    private final EventSet.WriterBuilder<KeyT, ValueT> eventSetWriterBuilder;
    ...
    private final Optional<Function<RangeProperties, StreamRangeEventWriter<KeyT, ValueT>>> writerFactory;
    ...
    WriterImpl(String writerName,
               ...
               StorageServerClientManager clientManager,
               OrderedScheduler scheduler,
               Optional<Function<RangeProperties, StreamRangeEventWriter<KeyT, ValueT>>> writerFactory) {
        ...
        this.eventSetWriterBuilder = EventSet.<KeyT, ValueT>newWriterBuilder()
            .withEventSetType(EventSetType.DATA)
            .withBufferSize(writerConfig.maxBufferSize()) // 128k
        ...
        this.rangeClients = new StreamRangeClients(
    ...
    StreamRangeEventWriter<KeyT, ValueT> newStreamRangeEventWriter(RangeProperties rProps) {
        if (writerFactory.isPresent()) {
            return writerFactory.get().apply(rProps);
        }
    ...
    public CompletableFuture<WriteResult> write(WriteEvent<KeyT, ValueT> event) {
        ...
        return writeFuture.thenApply(pos -> WriteResultImpl.of(pos));
    }
    ...
    private void handleStreamRangeFenced() {
        ...
        for (PendingEvent<KeyT, ValueT> event : pendingEvents) {
            try {
                WriteEvent<KeyT, ValueT> writeEvent = event.getAndReset();
