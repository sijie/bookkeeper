...
public class StreamRangeEventSetReaderImpl
        ...
        if (readResponse.getReadSessionId() < readSessionId.get()) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("Received events for range {} from older session {} : current session = {}",
                    new Object[]{range, readResponse.getReadSessionId(), readSessionId.get()});
            }
            return;
        }
    ...
    private void readMoreBytes(long moreBytes) {
        ...
        if (log.isDebugEnabled()) {
            log.debug("read more bytes {}, total = {}", moreBytes, newSize);
        }
    }
    ...
    private void connect(CompletableFuture<Void> connectFuture) {
        ...
        scChannel.getStorageContainerChannelFuture().whenCompleteAsync((rsChannel, cause) -> {
            ...
            log.info("Connected to range {} @ storage container {}", range, scChannel.getStorageContainerId());

            this.requestObserver =
                rsChannel.getStreamReadService().read(this);

            long nextReadBytes = getNextReadBytes();
            this.requestObserver.onNext(ReadRequest.newBuilder()
                .setSetupReq(SetupReaderRequest.newBuilder()
                    ...
                    .setRangeOffset(nextRangeOffset)
                    .setRangeSeqNum(nextRangeSeqNum))
                .build());
            long newSize = requestedSize.addAndGet(nextReadBytes);
            if (log.isDebugEnabled()) {
                log.debug("read more bytes {}, total = {}", nextReadBytes, newSize);
            }
        }, executor);
    }
