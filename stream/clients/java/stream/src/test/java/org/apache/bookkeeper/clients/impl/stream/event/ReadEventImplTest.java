 ...
 */
public class ReadEventImplTest {

    private final ReadEventImpl.Recycler<ByteBuf, ByteBuf> recycler;

    public ReadEventImplTest() {
        this.recycler = new ReadEventImpl.Recycler<>();
    }
    ...
    public void testNewEvent() {
        ...
        ReadEventImpl<ByteBuf, ByteBuf> event = recycler.newEvent();
        assertNull(event.key());
        assertNull(event.value());
        assertEquals(-1L, event.timestamp());
        assertNull(event.stream());
        ...
        event.setName("test-stream");
        event.setPosition(Position.HEAD);
        ...
        ReadEventImpl<ByteBuf, ByteBuf> newEvent = recycler.newEvent();
