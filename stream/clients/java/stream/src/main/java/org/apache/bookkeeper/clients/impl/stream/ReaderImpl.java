...
import org.apache.bookkeeper.clients.impl.stream.range.StreamRangeEventReaderFactoryImpl;
...
class ReaderImpl<KeyT, ValueT>
    ...
    private final StreamRangeEventReaderFactory rangeEventReaderFactory;
    ...
    private volatile long readerRevision;
    @Getter(AccessLevel.PACKAGE)
    ...
    ReaderImpl(String readerName,
        ...
        this(
            ...
            scheduler,
            readGroupCommandar,
            new StreamRangeEventReaderFactoryImpl(
                new StreamRangeClients(rsClientManager.getStorageContainerChannelManager(), scheduler),
                scheduler)
        );
    }
    ...
    ReaderImpl(String readerName,
               ...
               OrderedScheduler scheduler,
               StreamObserver<ReadGroupCommand> readGroupCommandar,
               StreamRangeEventReaderFactory rangeReaderFactory) {
        ...
        this.rangeEventReaderFactory = rangeReaderFactory;
    ...
    synchronized int getNumPendingReaders() {
        return readerQueue.size();
    }
    ...
    CompletableFuture<Reader<KeyT, ValueT>> start() {
        ...
        executor.submit(() -> {
            ...
            state = State.ADDING;
            ReadGroupCommand addReaderCommand = newCommandBuilder(ReadGroupCommand.Op.ADD_READER)
                .setAddCmd(AddReaderCommand.newBuilder())
                .build();
            readGroupCommandar.onNext(addReaderCommand);
        });
        return future;
    }
    ...
    CompletableFuture<Reader<KeyT, ValueT>> stop() {
        ...
        log.info("Stopping reader ({}) ...", readerName);
        future.whenCompleteAsync((value, cause) -> {
            if (null != cause) {
                readGroupCommandar.onError(cause);
            } else {
                log.info("Reader ({}) is successfully stopped.", readerName);
        ...
        executor.submit(() -> {
            if (State.ADDED != state) {
                future.completeExceptionally(
                    new IllegalStateException("Try to stop reader (" + readerName + ") that wasn't started yet."));
                return;
            }
    ...
    protected void closeAsyncOnce(CompletableFuture<Void> closeFuture) {
        executor.submit(() -> {
            if (null == error && State.ADDED == state) {
    ...
    private void closeReadersAndComplete(CompletableFuture<Void> closeFuture) {
        ...
        state = State.CLOSE;
        log.info("Reader ({}) closing all the open stream range readers ...", readerName);
        FutureUtils.proxyTo(
    ...
    protected CompletableFuture<Void> closeReaders(List<StreamRangeEventReader<KeyT, ValueT>> readers) {
        ...
        return FutureUtils.collect(closeFutures)
            .thenApplyAsync((value) -> {
                log.info("Reader ({}) successfully close {} range readers.", readerName, closeFutures.size());
                return null;
            });
    }
        ...
        clientManager.openMetaRangeClient(streamProps).getActiveDataRanges().thenAccept(streamRanges -> {
            ...
            for (RangePosition rangePos : streamPos.getRangePositionsList()) {
                ...
                if (null != rangeProps) {
                    ...
                    StreamRangeEventReader<KeyT, ValueT> reader =
                        rangeEventReaderFactory.createRangeEventReader(
                            streamConfig,
                            streamProps,
                            rangeProps,
                            rangePos,
    ...
    private synchronized void scheduleRead() {
        StreamRangeEventReader<KeyT, ValueT> reader = readerQueue.poll();
        while (null != reader) {
            startRead(reader);
            reader = readerQueue.poll();
        }
    }
    ...
    public ReadEvents<KeyT, ValueT> readNext(long waitTime, TimeUnit timeUnit)
        ...
        if (isClosed()) {
            throw new StreamApiException(Code.READER_REINIT_REQUIRED,
                new ObjectClosedException("reader-" + readerName));
        }
